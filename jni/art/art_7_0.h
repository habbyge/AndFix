...
public:
	...
	//
	// ArtFields are allocated as a length prefixed ArtField array, 
	// and not an array of pointers to ArtFields.
	uint64_t ifields_;

	// Pointer to an ArtMethod length-prefixed array. All the methods where this 
	// class is the place where they are logically defined. This includes all 
	// private, static, final and virtual methods as well as inherited default 
	// methods and miranda methods.
	//
	// The slice methods_ [0, virtual_methods_offset_) are the direct (static, 
	// private, init) methods declared by this class.
	//
	// The slice methods_ [virtual_methods_offset_, copied_methods_offset_) 
	// are the virtual methods declared by this class.
	//
	// The slice methods_ [copied_methods_offset_, |methods_|) are the methods 
	// that are copied from interfaces such as miranda or default methods. 
	// These are copied for resolution purposes as this class is where they are 
	// (logically) declared as far as the virtual dispatch is concerned.
	//
	// Note that this field is used by the native debugger as the unique 
...
public:
	...
	// declaring_class_->dex_cache_ Offset to the CodeItem.
	...
	// methods the index is into the declaringClass.directMethods, for 
	// virtual methods the vtable and for interface methods the ifTable.
	uint16_t method_index_;

	// The hotness we measure for this method. Incremented by the interpreter. 
	// Not atomic, as we allow missing increments: if the method is hot, 
	...
	struct PtrSizedFields {
		// Short cuts to declaring_class_->dex_cache_ member for 
		// fast compiled code access.
		...
		// to resolve the JNI function, or the profiling data for non-native 
		// methods, or an ImtConflictTable.
		void* entry_point_from_jni_;

		// Method dispatch from quick compiled code invokes this pointer 
		// which may cause bridging into the interpreter.
